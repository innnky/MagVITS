...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    net_g.train()
    net_d.train()
    for batch_idx, (ssl, spec, spec_lengths, y, y_lengths, text, text_lengths, bert,spk_emb_padded,duration) in tqdm(enumerate(train_loader)):
        ...
        spk_emb_padded = spk_emb_padded.cuda(rank, non_blocking=True)

        with autocast(enabled=hps.train.fp16_run):
            y_hat, attn, ids_slice, x_mask, z_mask, \
                (z, z_p, m_p, logs_p, m_q, logs_q), z_q, quantize_loss, dur_loss, pred_dur, prosody_predict_loss = net_g(text, text_lengths, spec, spec_lengths, ssl, duration, bert, spk_emb_padded)
        ...
        with autocast(enabled=hps.train.fp16_run):
            ...
            with autocast(enabled=False):
                ...
                loss_dur = torch.sum(dur_loss.float())
                loss_fm = feature_loss(fmap_r, fmap_g)
                loss_gen, losses_gen = generator_loss(y_d_hat_g)
                loss_gen_all = loss_gen + loss_fm + loss_mel + loss_kl  + quantize_loss + loss_dur + prosody_predict_loss
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                losses = [loss_disc, loss_gen, loss_fm, loss_mel, quantize_loss, loss_kl]
                ...
                scalar_dict.update(
                    {"loss/g/fm": loss_fm, "loss/g/mel": loss_mel, "loss/g/kl": loss_kl,
                     "loss/g/quantize_loss": quantize_loss,'dur_loss':loss_dur,'prosody_predict_loss':prosody_predict_loss})
                ...
                image_dict = {
                    ...
                    "all/z_q": utils.plot_spectrogram_to_numpy(z_q[0].data.cpu().numpy()),
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    print("Evaluating ...")
    with torch.no_grad():
        for batch_idx, (ssl, spec, spec_lengths, y, y_lengths, text, text_lengths, bert,sid,duration) in enumerate(eval_loader):
            print(111)
            ...
            y_hat, mask, *_ = generator.module.reconstruct(text, text_lengths, spec, spec_lengths, ssl,duration, bert,sid, noise_scale=0.5)
            ...
            audio_dict.update({f"gt/audio_{batch_idx}": y[0, :, :]})
